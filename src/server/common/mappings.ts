...
import { GithubIssue, GitHubRepository } from "../trpc/router/repo";
...
export default function mapRepository(repo: GitHubRepository): Repository {
  return {
    ...
    issues_stat: {
      ...
      time_since_last_closed: daysSince(
        repo.closed_issues.items
          .map((x) => x.closed_at)
          .sort((a, b) => delta(a, b))[0],
        -1
      ),
    },
...
function averageTimeOpen(
  closed: Response<GithubIssue[]>,
  open: Response<GithubIssue[]>
): number {
